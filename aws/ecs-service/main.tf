resource "aws_ecs_service" "this" {
  ...
  lifecycle {
    ...
    # https://github.com/hashicorp/terraform-provider-aws/issues/19519#issuecomment-1648426533
    postcondition {
      condition     = self.task_definition == aws_ecs_task_definition.this.arn
...
resource "aws_appautoscaling_scheduled_action" "scheduled" {
  for_each = var.has_autoscaler && var.scheduled_actions_enabled ? var.scheduled_scaling_actions : {}

  name               = "${aws_ecs_service.this.name}-${each.key}"
  service_namespace  = aws_appautoscaling_target.this[0].service_namespace
  resource_id        = aws_appautoscaling_target.this[0].resource_id
  scalable_dimension = aws_appautoscaling_target.this[0].scalable_dimension
